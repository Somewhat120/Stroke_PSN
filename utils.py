...
def train_resample(X_train, Y_train, seed):
    # X_train = df_train[:, 1:]
    # Y_train = df_train[:, 0]
...
def valid_resample(df_valid, seed):
    X_valid = df_valid[:, 1:]
    Y_valid = df_valid[:, 0]
    sm_ = SMOTETomek(random_state=seed,
                     sampling_strategy=0.5)
    X_va, y_va = sm_.fit_resample(X_valid, Y_valid)
    return X_va, y_va
...
def calculate_specificity(label, pred_label):
    confusion = confusion_matrix(label,pred_label)
    TP = confusion[1, 1]
    TN = confusion[0, 0]
    FP = confusion[0, 1]
    FN = confusion[1, 0]
...
def get_metrics(real_score, predict_score, threshold='auto'):
    ...
    if threshold == 'auto':
        ...
        f1_scores = f1_score_list[max_index]
        accuracy = accuracy_list[max_index]
        specificity = specificity_list[max_index]
        recall = recall_list[max_index]
        precision = precision_list[max_index]
        return auc[0, 0], aupr[0, 0], accuracy, f1_scores, precision, recall, specificity
    else:
        auc, aupr = get_metrics_auc(real_score, predict_score)
        predict_label = np.array([1 if x >= threshold else 0 for x in predict_score])
        precision = precision_score(real_score, predict_label)
        recall = recall_score(real_score, predict_label)
        f1 = f1_score(real_score, predict_label)
        accuracy = accuracy_score(real_score, predict_label)
        specificity = calculate_specificity(real_score, predict_label)
...
def plot_curve(args, print_list, fold):
    print_list = np.array(print_list).T
    loss = print_list[1]
    auc_train = print_list[2]
    aupr_train = print_list[3]
    auc_val = print_list[4]
    aupr_val = print_list[5]
    idx = print_list[0]
    plt.plot(idx, loss, label='Loss')
    plt.plot(idx, auc_train, label='Train AUC')
    plt.plot(idx, aupr_train, label='Train AUPR')
    plt.plot(idx, auc_val, label='Val AUC')
    plt.plot(idx, aupr_val, label='Val AUPR')
    plt.legend()
    plt.savefig(os.path.join(args.saved_path,
                             'training_curve_{}.png'.format(fold)))
    plt.clf()
